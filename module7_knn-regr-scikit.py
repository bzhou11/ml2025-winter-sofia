...
class knn_reg:
	...
	def calculate_output_Y(self, X, k, N):
		if k <= N:
			...
			X_train = self.arrays[:, 0].reshape((N, 1))
			y_train = self.arrays[:, 1]
			variance_y_label = np.var(y_train)
			knn_regressor.fit(X_train, y_train)
			Y = knn_regressor.predict([[X]])
			return "variance of labels in the training dataset: " + str(variance_y_label) + "\n" + "output Y: " + str(Y)
		else: 
			y_train = self.arrays[:, 1]
			variance_y_label = np.var(y_train)
